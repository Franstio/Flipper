    ...
    internal class DashboardPresenter : DashboardMVP.IPresenter, IDisposable
    {
        ...
        public async Task<ProcessResultModel[]> RunProcess()
        {
            ...
            try
            {
                ...
                {
                    await File.WriteAllTextAsync(Path.Combine(Settings.Default.DebugLogPath, $"Top-UV-debug-{DateTime.Now.ToString("yyyy-MM-dd_HHmmss")}.log"), JsonSerializer.Serialize(record[0]));
                    throw new Exception("Top UV Fail but no data detected");
                    
                }
                if (view.BottomUVDecision == "FAIL" && view.BottomUVRecord.Count < 1)
                {
                    await File.WriteAllTextAsync(Path.Combine(Settings.Default.DebugLogPath, $"Bottom-UV-debug-{DateTime.Now.ToString("yyyy-MM-dd_HHmmss")}.log"), JsonSerializer.Serialize(record[1]));
                    throw new Exception("Bottom UV Fail but no data detected");

                }
                if (view.TopWhiteDecision == "FAIL" && view.TopWhiteRecord.Count < 1)
                {
                    await File.WriteAllTextAsync(Path.Combine(Settings.Default.DebugLogPath, $"Top-White-debug-{DateTime.Now.ToString("yyyy-MM-dd_HHmmss")}.log"), JsonSerializer.Serialize(record[2]));
        ...
        public async Task WriteLog(List<RecordModel> records, string sn)
        {
            if (records is null)
                return;
